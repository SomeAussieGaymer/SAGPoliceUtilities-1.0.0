...
namespace SAGPoliceUtilities.Commands.JailCommands
{
    public class Jail : IRocketCommand
    {
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            var jailName = SAGPoliceUtilities.Instance.Configuration.Instance.Jails.FirstOrDefault(x => x.Name.ToLower() == command[1].ToLower());
            ...
            if (jailedPlayer.IsAdmin || jailedPlayer.HasPermission("jailimmune"))
            {
            ...
            if (SAGPoliceUtilities.Instance.JailTimeService.IsPlayerJailed(jailedPlayer.CSteamID.ToString(), out jailedTime))
            {
                ...
                var jail = SAGPoliceUtilities.Instance.Configuration.Instance.Jails.FirstOrDefault(x => x.Name == jailedTime.JailName);
            ...
            SAGPoliceUtilities.Instance.JailTimeService.RegisterJailedUser(jailedPlayer.CSteamID.ToString(), jailName, jailTime, jailReason);
